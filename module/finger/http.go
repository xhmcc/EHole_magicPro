...
func httprequest(url1 []string, proxy string) (*resps, error) {
	...
	// 检查是否是400 The plain HTTP request was sent to HTTPS port错误
	if strings.Contains(httpbody, "400 The plain HTTP request was sent to HTTPS port") {
		// 将http改为https重试
		httpsURL := strings.Replace(url1[0], "http://", "https://", 1)
